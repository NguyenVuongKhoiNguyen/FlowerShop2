...
CREATE PROCEDURE sp_FilteredPaginatedProducts
    @MinPrice FLOAT = NULL,
    @MaxPrice FLOAT = NULL,
    @CategoryId INT = NULL,
...
BEGIN
    ...
    ORDER BY 
        CASE WHEN @SortOrder = 'PRICE-ASC' THEN (CostPrice * RetailPercentage) END ASC,
        CASE WHEN @SortOrder = 'PRICE-DESC' THEN (CostPrice * RetailPercentage) END DESC,
		CASE WHEN @SortOrder = 'SALES-ASC' THEN Sales END ASC,
		CASE WHEN @SortOrder = 'SALES-DESC' THEN Sales END DESC,
		Id DESC
...
CREATE PROCEDURE sp_CountFilteredProducts
    @MinPrice FLOAT = NULL,
    @MaxPrice FLOAT = NULL,
    @CategoryId INT = NULL,
    @ProductName NVARCHAR(50) = NULL,
...
CREATE PROCEDURE sp_FilteredPaginatedCategories
    @InputStr NVARCHAR(50) = NULL,
...
BEGIN
    ...
    WHERE
        (
            @InputStr IS NULL OR Name LIKE '%' + @InputStr + '%'
        )
...
CREATE PROCEDURE sp_CountFilteredCategories
    @InputStr NVARCHAR(50) = NULL
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE PROCEDURE sp_FilteredPaginatedAccounts
...
BEGIN
    ...
    ORDER BY
        CASE WHEN @SortOrder = 'ASC' THEN CreateDate END ASC,
        CASE WHEN @SortOrder = 'DESC' THEN CreateDate END DESC
...
CREATE PROCEDURE sp_CountFilteredAccounts
    @Username VARCHAR(50) = NULL,
    @Fullname NVARCHAR(50) = NULL,
    @Email NVARCHAR(50) = NULL,
    @Activated BIT = NULL
...
CREATE PROCEDURE sp_GetOrdersByUserAndDate
    @Username VARCHAR(50) = NULL,
...
FROM Orders o
WHERE 
    (@Username IS NULL OR o.Username LIKE '%' + @Username + '%')
    AND (@Fullname IS NULL OR o.Fullname LIKE '%' + @Fullname + '%')
    AND (@FromDate IS NULL OR o.CreateDate >= @FromDate)
    AND (@ToDate IS NULL OR o.CreateDate < DATEADD(DAY, 1, @ToDate))
...
CREATE PROCEDURE sp_CountFilteredOrders
    @Username VARCHAR(50) = NULL,
    @FromDate DATETIME = NULL,
    @ToDate DATETIME = NULL
AS
...
update products set sales = 8 where id = 1
update products set sales = 7 where id = 2
update products set sales = 6 where id = 3
update products set sales = 5 where id = 4
update products set sales = 4 where id = 5
update products set sales = 3 where id = 6
update products set sales = 2 where id = 7
update products set sales = 1 where id = 8
GO
